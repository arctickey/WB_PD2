...
- **VIM**: Package for visualization and imputation of missing values.
...
- Categorical veriable are convert to lowercase when  it reduce number of unique categories.
- Removing columns if they are not containing any information (for example all observation have the same value).
- Features like date or location are transform to help algoritm understand them (date for example is splited to three column 'year' , 'day' , 'month').
...
First we use Mode/median inputation it is very simple method and it is used as base result for more complex algoritm to improve.
