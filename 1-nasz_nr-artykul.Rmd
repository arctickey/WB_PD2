...
- **softImpute**[@R-softImpute]: Package for matrix imputation with nuclear-norm regularization. Algorithm works like EM, solving optimization problem using soft-thresholded SVD algorithm.  Works only with continuous variables.
- **VIM**[@VIM2016]: Package for visualization and imputation of missing values. Offers iterative robust model-based imputation (IRMI). In each iteration, one variable is used as response variable and remaining variables as the regressors.

#### Data source and preparation
For test purposes we used 11 dataset form OpenML library [@R-OpenML]. Every dataset is designed for binary classification and most of them contains numerical and categorical features.  
