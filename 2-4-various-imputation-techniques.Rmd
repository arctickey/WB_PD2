...
Imputation method from mice package don't require any form of help, because they can impute numeric and categorical features.
...
The last method which we covered is missMDA which also can be used to impute numeric and categorical features.
...
As a second approach, for each data set we decided to rank imputation methods according to the models scores, and then award points for each place. As a score of models for imputation method on the given data set, we understand as before the average F1 score made by all models on the imputation result. This approach allows us to give the lowest rank for the imputation methods which failed on a specific set and compare them fairly to others. As a result, we sum up all of the points and the final score is the sum of the points across all data sets. Of course, the method with the lowest amount of points wins.
...
Scores achieved here resemble these achieved before. The order is not the same, but the winner remains the same. Once again median/mode imputation managed to perform very well, outperforming many complicated algorithms. We can say that our scores are stable, so conclusions taken from these scores can be taken seriously.
