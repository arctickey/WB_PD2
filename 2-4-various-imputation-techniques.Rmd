...
After the long and tedious process of data imputation it is finally time to evaluate our methods of imputation. Methods were trained and tested on 14 datasets and evaluated strictly using 2 methods mentioned earlier. Before score analysis it is worth mentioning that all algorithms were tested with optimal parameters, so score should be quite meaningful. Table below presents average metrics achieved by model, depending on imputation method. As described earlier, we decided to use two measures of algorithm effectiveness:
...
First of all, we can think about median imputation as kind of our baseline because it will be useful later. Suprisngly it seems to perform quite well, amongst the others, often quite sophisticated methods. It achieved over 80% of accuracy on average. However, it is hard to decide whether it is a high score or not, because we are only able to compare algorithms between the others. To say anything more meanigful about our methods we shall look at the distribution of the scores in order to make a better analysis of performance.\

\newpage
##### Distributions of scores

```{r echo=FALSE, message=FALSE, warning=FALSE,fig.width=3, fig.height=3.5,fig.show='hold',fig.align='center'}
p1<-ggplot(scores, aes(x=Method, y=Acc, color=Method)) +
  geom_boxplot()+theme_minimal()+labs(x="",y="")+theme( axis.text.x=element_blank())+ggtitle('Accuracy')
p2<-ggplot(scores, aes(x=Method, y=F1, color=Method)) + 
  geom_boxplot()+theme_minimal()+labs(x="",y="")+theme(axis.text.x=element_blank())+ggtitle('F1')
p1
p2
...
So to begin with we decided to  treat the median as the basic form of imputation and we will compare it to all other methods. We want to check how much the average prediction measured made by other algorithms differed from the median. As one prediction, we understand the average F1 score made by of all imputation methods on one dataset. 
...
colnames(order_scores1) <- c('Method','Score')
...
scores1 <- order_scores2 %>% group_by(Method) %>% summarise(score = sum(diff)/un)
x <- scores1[scores1$Method!='aaamedian',]
x <- scores1[x$Method!='median',]
...
\newpage
Results achieved here are quite surprising. First of all the maximum gain in F1 score is only 0.02 and it is achieved by softimpute. But what can bewilder the most is the fact that only 2 out of 5 algorithms managed to perform better than median. These scores can raise questions about the whole purpose of using sophisticated algorithms, but of course we are unable to say anything harsh about them yet. We can definately note that we need to be we careful while using these algorithms, because the may lead to huge loss in scores, even up to -0.11, as here it is shown with missmda. It is worth noting the order of the algorithms here, as we will use another method to compare the scores. 


#### Second approach

As a second approach we decide to simply rank each algorithm score on each dataset, and than award points for each place. As a result we sum up all of the points and the final score is the sum of the points across all datasets. Of course the algorithm with the lowest amount of points wins.

```{r echo=FALSE, message=FALSE,warning=FALSE}
knitr::kable(order_scores1)
```

Scores achieved here resemble these achieved here. The order is not the same, but the winner remains the same. Once again median imputation managed to perform very well, outperforming many complicated algorithms. We can say that our scores are stable, so conclusions taken from these scores can be taken seriously.


#### Times
As a final tool of comparing algorithms we decided to compare times of imputation. Obviously even the algorithm with the best score, butwith awful imputation time is considered useless, so this is important to take that factor to final evaluation.



```{r echo=FALSE, message=FALSE,warning=FALSE,warning=FALSE,fig.width=4,fig.height=3,fig.show='hold',fig.align='center'}
times <- read.csv('./final_score/final.csv')
p3<-ggplot(times, aes(x=X, y=V2, color=X)) +
  geom_boxplot()+theme_minimal()+labs(x="",y="Seconds")+ggtitle('Times')+theme(legend.title=element_blank())+theme(axis.text.x=element_blank())
p3
```

As we can see times of imputations made by each algorithm can vary heavily. There is no point in creating rankings like we did to analyse scores achieved by algorithms, but it is simply worth noting that our previous winner - SoftImpute is very quick, its plot looks the same as the Median plot, which is quite an achievment.
...
Summing up all of our work we can say, that we managed to find plenty of interesting things about all of these imputation algorithms and the way they deal we different data. However it is hard to issue the final judgments, because of the fact that we only had 14 datasets available. Despite that, something definately can be said. First of all it is worth noting how well simple methods of imputation have managed to perform. Median imputation was tough competitor and have been outperformed rarely in our tests. Second issue is the matter of choosing the optimal metric for checking the performance of imputation algorithms. We have shown 2 different approaches and scores they achieved differed slightly. Taking that into consideration we cannot say that we have a clear winner in our competition, but we can give some sort of an advice for all interesting in imputing their datasets. The advice is quite simple, but what we have managed to show quite powerful. The case is to start imputing your dataset with the most basic method, which is median imputing, and then trying to beat its score with different algorithm, for example SoftImpute, which in our tests managed to perform quite well.
