...
all_imputation <- function(data,target){
  ...
  time_mice <- Sys.time()
  data_mice <- data
  data_mice[train_set,] <- prepareMice(data_mice[train_set,])$data
  data_mice[test_set,] <- prepareMice(data_mice[test_set,])$data
  ...
  type_of_imputation <- list('median', 'mice')
  datasets <- list(data_median, data_mice)
  times <- list(time_median, time_mice)
